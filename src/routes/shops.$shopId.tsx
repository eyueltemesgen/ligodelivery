...
import { shopProductsQuery, shopQuery } from "@/lib/queries";
import { StorageImage } from "@/lib/media";
import { ETB, isShopOpen } from "@/lib/format";
...
export const Route = createFileRoute("/shops/$shopId")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse the menu and order delivery from this Bishoftu shop on Ligo." },
...
function ShopDetail() {
  ...
  if (isLoading) return <div className="container-ligo py-16 text-muted-foreground">Loading shop…</div>;
  ...
  return (
    <div>
      <div className="relative h-56 w-full overflow-hidden bg-surface">
        <StorageImage path={shop.cover_url ?? shop.image_url} alt={shop.name} className="h-56 w-full object-cover" />
      </div>
      <div className="container-ligo -mt-10 pb-12">
        <div className="rounded-xl border border-border bg-card p-5 shadow-pop">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className={`rounded-full px-2 py-1 text-xs font-semibold ${open ? "bg-primary-soft text-accent-foreground" : "bg-muted text-muted-foreground"}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{shop.delivery_time_min} min · {ETB(shop.delivery_fee)} delivery</span>
        ...
        {products.length === 0 ? (
          ...
          <div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {products.map((p) => <ProductCard key={p.id} product={p} shopName={shop.name} />)}
          </div>
        )}
      </div>
    </div>
