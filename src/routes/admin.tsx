...
import { PROOF_BUCKET, StorageImage, uploadImage, useMediaUrl } from "@/lib/media";
...
function RidersAdmin() {
  ...
  const approve = async (id: string, value: boolean) => {
    ...
    await notify(
      id,
      value ? "Rider approved" : "Rider access paused",
      value ? "You can now accept deliveries." : "Contact the Ligo team.",
      "rider",
    );
      ...
      {riders.map((r) => (
        ...
        >
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {r.profile?.phone} · {r.vehicle_type} · ID {r.national_id || "—"}
            </p>
            <p className="text-xs text-muted-foreground">{r.is_online ? "Online" : "Offline"}</p>
            <div className="mt-1 flex gap-3 text-xs">
              <DocLink path={r.id_document_url} label="National ID" />
              <DocLink path={r.license_document_url} label="License" />
            </div>
          </div>
...
function DocLink({ path, label }: { path: string | null; label: string }) {
  ...
  return (
    <a href={url} target="_blank" rel="noreferrer" className="font-medium text-primary underline">
      {label}
    </a>
  );
}
